...
data class PlayerData(
    ...
    private fun rightClickTick() {
        val fireDelay = plugin.settings.fireDelay

        if (shotTime >= 0) {
            val shots = max(1, shotTime / fireDelay)
            for (i in 0 until shots) {
                fire()
            }
            shotTime -= fireDelay * shots
        }
    }
    ...
    fun syncTick(dt: Long) {
        //player.sendMessage(" > ${shotTime - dt} -> $shotTime")
        player.sendActionBar(Component.text("$holdingRClick | $shotTime (${System.currentTimeMillis() - shotTime})"))
    }
    ...
    fun asyncTick(dt: Long) {
        ...
        if (applied.manhattanLength >= EPSILON) {
            PacketEvents.getAPI().playerManager.sendPacket(player, WrapperPlayServerPlayerPositionAndLook(
